...
- 下書きの公開（`draft` を外して保存するのを、保存ボタンの隣からワンタップで）
- 記事の削除
...
下書きを出力しないので、書きかけをコミットしても公開はされません。公開したくなったら
「公開する」ボタンを押します（`draft` を外して保存するのと同じことですが、チェック
ボックスは折りたたまれた設定パネルの中にあり、記事をインターネットに出す唯一の操作の
置き場所としては不適切なので、保存ボタンの隣に出しています）。
...
- 記事を削除しても、その記事用の画像ディレクトリは残ります。contents API は
  1 ファイルごとに 1 コミットを作るうえ、他の記事から参照されている画像を消してしまう
  ほうが、数 KB の孤児が残るよりずっと悪い結果になるためです。どちらにしても
  git の履歴には残るので、必要なら復元できます。
